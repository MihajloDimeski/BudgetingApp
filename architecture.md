...
2. **Editing & Updating (e.g., Paycheck Raises)**: Recurring transactions (like paycheck income sources) are editable. Passing `edit_recurring_id` as a query parameter to `/budgets` retrieves the transaction and populates the budget form in "Edit Mode". Submitting the form posts to `/recurring/update/<int:id>`, which updates the `amount`, `description`, `frequency`, `next_due_date`, `currency`, and `category_id`, as well as optional manual override offsets (`total_avail_override`, `spent_override`, `remaining_override`), committing the changes to the database. These overrides act as base starting points for the month. To prevent "double-counting" existing transactions of the current month during saves, the update endpoint dynamically converts the user's submitted values into starting base offsets by subtracting/adding the current month's transaction spent total (specifically, `spent_override = input_value - current_spent` and `remaining_override = input_value + current_spent`). Consequently, the dashboard displays exactly the values entered by the user, and subsequent transactions will continue to dynamically adjust them.
